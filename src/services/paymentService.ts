    ...
    async (bail: (err: Error) => void, attempt: number) => {
      try {
        ...
        if (!paymentInitiatedAt && !productStillLive) {
          console.warn(
            `[PAYMENT_SERVICE] ❌ Order ${order.id} - Product offline and no payment start time found`
          );
        ...
        if (!productStillLive || !isWithinGrace) {
          console.warn(
            `[PAYMENT_SERVICE] ⚠️ Late or expired payment for order ${order.id}`
          );
