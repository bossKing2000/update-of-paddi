...
import { Prisma, PrismaClient, Role, Category, OrderStatus, PaymentStatus } from "@prisma/client";
import { faker } from "@faker-js/faker";
import { redisNotifications, redisProducts, redisSearch, ShopCartRedis } from "../lib/redis";
...
async function clearRedisCaches() {
  ...
  const clients = [redisProducts, redisSearch, redisNotifications, ShopCartRedis];
...
async function main() {
  ...
  function generateSafeUserIdentifiers(role: string, index: number) {
    const cleanName = faker.person.firstName().toLowerCase().replace(/[^a-z0-9]/g, '');
  ...
  for (let i = 0; i < totalVendors; i++) {
    ...
    usersData.push({
      ...
      preferences: faker.helpers.arrayElements(["DESSERT", "DRINK", "DINNER","BREAKFAST","LUNCH"]),
      isEmailVerified: true,
    });
  }
  ...
  // ==============================
  const deliveryUsers = await prisma.user.findMany({ where: { role: Role.DELIVERY } });
  const deliveryProfilesData: Prisma.DeliveryPersonCreateManyInput[] = deliveryUsers.map((user) => ({
    userId: user.id,
  ...
  const activeVendors = faker.helpers.arrayElements(allVendors, Math.floor(allVendors.length * VENDOR_ENGAGEMENT));
  const allCustomers = await prisma.user.findMany({ where: { role: Role.CUSTOMER } });
  const customers = faker.helpers.arrayElements(allCustomers, Math.floor(allCustomers.length * CUSTOMER_ENGAGEMENT));

  for (const vendor of activeVendors) {
    const totalProducts = faker.number.int({ min: 5, max: MAX_PRODUCTS_PER_VENDOR });
    ...
    for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
      ...
      if (productsData.length) await prisma.product.createMany({ data: productsData });
      if (optionData.length) await prisma.productOption.createMany({ data: optionData });
      ...
      if (scheduleData.length) await prisma.productSchedule.createMany({ data: scheduleData });
...
// ==============================
const engagedCustomers = faker.helpers.arrayElements(allCustomers, Math.floor(allCustomers.length * CUSTOMER_ENGAGEMENT));
  ...
  for (let i = 0; i < ordersCount; i++) {
    ...
    const order = await prisma.order.create({
      data: {
        ...
        paidAt: isCompleted 
          ? new Date(orderDate.getTime() + faker.number.int({ min: 30000, max: 3600000 })) // 30 sec to 1 hour later
          : null,
        ...
        paymentStartedAt: new Date(orderDate.getTime() + faker.number.int({ min: 1000, max: 30000 })), // 1-30 seconds after order
    ...
    const paymentStartedAt = new Date(orderDate.getTime() + faker.number.int({ min: 1000, max: 30000 })); // 1-30 seconds after order
    const paymentCompletedAt = paymentStatus === PaymentStatus.SUCCESS 
      ? new Date(paymentStartedAt.getTime() + faker.number.int({ min: 1000, max: 60000 })) // 1 sec to 1 min later
      : null;
    ...
    await prisma.payment.create({
      data: {
        ...
        channel: faker.helpers.arrayElement(["card", "bank", "ussd", "mobile_money"]),
    ...
    if (orderStatus === OrderStatus.CANCELLED) {
      const cancellationTime = new Date(orderDate.getTime() + faker.number.int({ min: 60000, max: 3600000 })); // 1 min to 1 hour later
    ...
    if (orderStatus === OrderStatus.COMPLETED && faker.number.float({ min: 0, max: 1 }) < 0.3) {
      ...
      if (deliveryPersons.length > 0) {
        ...
        const assignedAt = new Date(orderDate.getTime() + faker.number.int({ min: 300000, max: 1800000 })); // 5-30 minutes after order
