...
//   description: string;
//   price: number;
//   images: string[]; // ✅ list view = max 1 image
//   category: string;
//   isLive: boolean;
//   liveUntil: Date | null;
//   productSchedule: {
...
//     const computeIsLive = (
//       schedule: ProductResponse["productSchedule"],
//       defaultIsLive: boolean
//     ): boolean => {
//       if (!schedule) return defaultIsLive;

//       const now = Date.now();
//       const goLive = schedule.goLiveAt
//         ? new Date(schedule.goLiveAt).getTime()
//         : 0;
...
//           price: true,
//           category: true,
//           thumbnail: true, // ✅ optimized list image
//           images: true,    // 👈 fallback for old products
//           popularityPercent: true,
//           isLive: true,
//           productSchedule: {
//             select: {
...
//     /* -------------------- STEP 2: COMPUTE LIVE -------------------- */
//     const productsWithLive = allProducts.map((p) => ({
//       ...p,
//       isLive: computeIsLive(p.productSchedule, p.isLive),
//     }));

//     /* -------------------- STEP 3: SORT (LIVE FIRST) -------------------- */
//     productsWithLive.sort((a, b) => Number(b.isLive) - Number(a.isLive));

//     /* -------------------- STEP 4: PAGINATE -------------------- */
//     const paginated = productsWithLive.slice(skip, skip + limit);
