...
export const getRecommendations = async (req: Request, res: Response) => {
  ...
  const recommendations =
    await recommendationService.getRecommendations(userId); // ✅ correct now
...
export const vendorAvailability = async (req: Request, res: Response) => {
  try {
    ...
    const suggestion =
      await vendorAvailabilityService.suggestAvailability(vendorId);
    res.status(200).json({ success: true, suggestion });
  } catch (err) {
    console.error(err);
    res.status(500).json({
      success: false,
      message: "Failed to suggest vendor availability",
    });
  }
};
