...
router.post('/logout', auth_middleware_1.authenticate, async (req, res) => { await (0, auth_controller_1.logout)(req, res); });
...
router.post('/logout-all-devices', auth_middleware_1.authenticate, async (req, res) => { await (0, auth_controller_1.logoutAllDevices)(req, res); });
//  GET /sessions
//  Lists every active session (device) for the current user
//  access Private
router.get('/sessions', auth_middleware_1.authenticate, async (req, res) => { await (0, auth_controller_1.getMySessions)(req, res); });
//  DELETE /sessions/:sessionId
//  Revokes one specific device's session without touching any others
//  access Private
router.delete('/sessions/:sessionId', auth_middleware_1.authenticate, async (req, res) => { await (0, auth_controller_1.revokeSession)(req, res); });
//  GET /login-history
//  Recent logins for the current user (device/location/time), for
//  spotting activity they don't recognize
//  access Private
router.get('/login-history', auth_middleware_1.authenticate, async (req, res) => { await (0, auth_controller_1.getLoginHistory)(req, res); });
//  GET /profile
...
//  access Public
router.post('/verify-reset-code', rateLimiter_middleware_1.authRateLimiter, async (req, res) => { await (0, auth_controller_1.verifyResetCode)(req, res); });
