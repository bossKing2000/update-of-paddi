...
exports.deleteSavedCard = exports.setDefaultCard = exports.getSavedCards = exports.saveCardToken = exports.verifyPaymentBeforeFulfillment = exports.requestRefund = exports.getAllPaymentsForUser = exports.confirmPayment = exports.webhookHandler = exports.initiateOrderPayment = exports.startPaymentSchema = void 0;
...
const paystackWebhookSchema = zod_1.z.object({
    event: zod_1.z.string(),
    data: zod_1.z.object({
        reference: zod_1.z.string(),
        amount: zod_1.z.number(), // in kobo
        ...
        authorization: zod_1.z.object({
            authorization_code: zod_1.z.string(),
            last4: zod_1.z.string(),
            brand: zod_1.z.string(),
            reusable: zod_1.z.boolean(),
            channel: zod_1.z.string().optional(),
        }).optional()
    }),
});
function validatePaystackSignature(rawBody, signature) {
    if (!signature)
        return false;
    const expected = crypto_1.default
        .createHmac('sha512', config_1.default.paystackSecret)
        .update(rawBody)
        .digest('hex');
...
const webhookHandler = async (req, res) => {
    try {
        ...
        if (authorization?.reusable && metadata?.userId) {
            try {
                await prisma_1.default.userPaymentMethod.upsert({
        ...
        const protectedUntilUtc = order.protectedUntil ? (0, time_1.toUtc)(order.protectedUntil) : null;
        const expiresAtUtc = payment.expiresAt ? (0, time_1.toUtc)(payment.expiresAt) : null;
        const isWithinProtection = protectedUntilUtc ? (0, time_1.isBeforeUtc)(now, protectedUntilUtc) : false;
        const isBeforeExpiry = expiresAtUtc ? (0, time_1.isBeforeUtc)(now, expiresAtUtc) : false;
        if (!isWithinProtection && !isBeforeExpiry) {
            console.warn(`[WEBHOOK] ⚠️ Late payment ${reference} — outside both protection & expiry windows.`);
            await prisma_1.default.payment.update({
                where: { reference },
        ...
        console.log(`[WEBHOOK] ✅ Payment ${reference} confirmed successfully for Order ${order.id}. (Protected=${isWithinProtection})`);
        return res.sendStatus(200);
    }
    catch (err) {
