...
class DashboardController {
    ...
    async getProductLiveControl(req, res) {
        try {
            if (!req.user)
                return res.status(401).json({ status: false, message: "Unauthorized", data: null });
            const service = new vendorDashboard_service_1.VendorDashboardService(req.user.id);
    ...
    // Clear vendor cache
    async clearVendorCache(req, res) {
        try {
            if (!req.user)
                return res.status(401).json({ status: false, message: "Unauthorized", data: null });
            const service = new vendorDashboard_service_1.VendorDashboardService(req.user.id);
    ...
    async getCustomerReturnRate(req, res) {
        try {
            if (!req.user)
                return res.status(401).json({ status: false, message: "Unauthorized", data: null });
            const service = new vendorDashboard_service_1.VendorDashboardService(req.user.id);
    ...
    async getRecentOrdersFromService(service) {
