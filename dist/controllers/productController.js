...
async function cleanupCloudinaryAssets(images, video) {
    try {
        ...
        if (video) {
            ...
            if (videoPublicId) {
                await cloudinary_1.v2.uploader.destroy(videoPublicId, { resource_type: 'video' });
...
const createProduct = async (req, res) => {
    try {
        ...
        if (!req.user || req.user.role !== "VENDOR") {
            res.status(403).json((0, codeMessage_1.errorResponse)("UNAUTHORIZED", "Only vendors can create products."));
            return;
        }
        ...
        if (typeof req.body.options === "string") {
            try {
                ...
                res.status(400).json((0, codeMessage_1.errorResponse)("INVALID_INPUT", "Invalid JSON in options field."));
                return;
            }
        }
        ...
        if (!parsed.success) {
            res.status(400).json((0, codeMessage_1.errorResponse)("VALIDATION_FAILED", JSON.stringify(parsed.error.issues, null, 2)));
            return;
        }
        ...
        if (imageUrls.length < 1 || imageUrls.length > 6) {
            res.status(400).json((0, codeMessage_1.errorResponse)("INVALID_IMAGE_COUNT", "Please upload between 1 and 6 images."));
            return;
        }
        ...
        // ── SMART CACHE UPDATE ───────────────────────────────────────────
        const firstPageKey = redisCacheTiming_1.CACHE_KEYS.PRODUCTS_ALL(1, 20);
        const categoryKey = `products:category:${category.toUpperCase()}:page=1:limit=20`;
        // Helper to rebuild cache for first page + category
        const rebuildCache = async (key, whereClause) => {
            const total = await prisma_1.default.product.count({ where: whereClause });
            ...
            await redis_1.redisProducts.set(key, JSON.stringify({
                data: products,
                pagination: {
                    total,
                    page: 1,
            ...
            }), { EX: redisCacheTiming_1.CACHE_TTLS.PRODUCTS_ALL });
        };
        await Promise.all([
            rebuildCache(firstPageKey),
            rebuildCache(categoryKey, { category }),
            (0, clearCaches_1.clearProductCache)(product.id), // invalidate detail cache if any exists
        ]);
        // ── Invalidate search suggestions cache ──────────────────────────
        const suggestionKeys = await redis_1.redisSearch.keys("suggestions:*");
        if (suggestionKeys.length)
            await redis_1.redisSearch.del(suggestionKeys);
        // ── Send success response ────────────────────────────────────────
        res
            .status(201)
            .json((0, codeMessage_1.successResponse)("PRODUCT_CREATED", "Product created successfully.", product));
    }
    catch (err) {
        console.error("Create product error:", err);
        res.status(500).json((0, codeMessage_1.errorResponse)("SERVER_ERROR", "Server error."));
    }
};
    ...
    try {
        ...
        if (categoryQuery && categoryQuery !== "ALL") {
            if (!Object.values(client_1.Category).includes(categoryQuery)) {
                res.status(400).json((0, codeMessage_1.errorResponse)("INVALID_CATEGORY", `Invalid category. Valid options: ${Object.values(client_1.Category).join(", ")}`));
                return;
            }
            where.category = categoryQuery;
        }
        /* -------------------- LIVE STATUS COMPUTATION -------------------- */
        const computeIsLive = (goLiveAt, liveUntil, graceMinutes, fallback) => {
            if (!goLiveAt || !liveUntil)
                return fallback;
            const now = Date.now();
            const grace = (graceMinutes ?? 0) * 60 * 1000;
            return (now >= new Date(goLiveAt).getTime() &&
                now <= new Date(liveUntil).getTime() + grace);
        };
        /* -------------------- CACHE KEY -------------------- */
        const cacheKey = categoryQuery && categoryQuery !== "ALL"
        ...
        if (cached) {
            ...
            allProducts = dbProducts.map((p) => ({
                ...
                isLive: computeIsLive(p.productSchedule?.goLiveAt ?? null, p.productSchedule?.takeDownAt ?? null, p.productSchedule?.graceMinutes ?? null, p.isLive),
                goLiveAt: p.productSchedule?.goLiveAt || null,
                liveUntil: p.productSchedule?.takeDownAt || null,
            }));
            totalCount = allProducts.length;
            await redis_1.redisProducts.set(cacheKey, JSON.stringify({ products: allProducts, total: totalCount }), { EX: redisCacheTiming_1.CACHE_TTLS.PRODUCTS_ALL });
        ...
        /* -------------------- RESPONSE -------------------- */
        res.json((0, codeMessage_1.successResponse)("PRODUCT_LIST", "Products fetched successfully", paginated, pagination));
    }
    catch (err) {
        console.error("❌ getAllProducts error:", err);
        await (0, clearCaches_1.clearProductCache)();
        res
            .status(500)
            .json((0, codeMessage_1.errorResponse)("SERVER_ERROR", "Internal server error"));
    }
};
...
const getProductById = async (req, res) => {
    ...
    try {
        ...
        (0, product_service_1.trackProductView)(productId).catch(console.error);
        // 🔹 Try to serve from cache first
        let cached = await redis_1.redisProducts.get(cacheKey);
        let productData;
        if (cached) {
            ...
            res.setHeader("X-Cache", "HIT");
            res.status(200).json((0, codeMessage_1.successResponse)("PRODUCT_FETCHED", "Product retrieved successfully.", productData));
            return;
        }
        ...
        if (!product) {
            res.status(404).json((0, codeMessage_1.errorResponse)("NOT_FOUND", "Product not found."));
            return;
        }
        ...
        await redis_1.redisProducts.set(cacheKey, JSON.stringify(productData), { EX: ttl });
        res.setHeader("X-Cache", "MISS");
        // 🔹 Send success response
        res.status(200).json((0, codeMessage_1.successResponse)("PRODUCT_FETCHED", "Product retrieved successfully.", productData));
    }
...
const updateProduct = async (req, res) => {
    ...
    try {
        ...
        if (!req.user || req.user.role !== "VENDOR") {
            res.status(403).json((0, codeMessage_1.errorResponse)("UNAUTHORIZED", "Only vendors can update products"));
            return;
        }
        ...
        if (!product || product.vendorId !== req.user.id) {
            res.status(403).json((0, codeMessage_1.errorResponse)("FORBIDDEN", "Unauthorized or product not found"));
            return;
        }
        ...
        if (!parsed.success) {
            ...
            res.status(400).json((0, codeMessage_1.errorResponse)("VALIDATION_ERROR", "Validation failed: check your input fields."));
            return;
        }
        ...
        const { name, description, price, category, archived, options, images: oldImagesFormat, // For backward compatibility
        ...
        if (isUsingOldFormat && isUsingNewFormat) {
            res.status(400).json((0, codeMessage_1.errorResponse)("CONFLICTING_FORMATS", "Cannot use both old format (images/video arrays) and new format (imageUpdates/videoUpdates)"));
            return;
        }
        // ============================================
        ...
        if (isUsingOldFormat) {
            ...
            if (invalidKeep.length > 0) {
                res.status(400).json((0, codeMessage_1.errorResponse)("INVALID_IMAGES", `Some images to keep don't exist in product: ${invalidKeep.slice(0, 3).join(', ')}${invalidKeep.length > 3 ? '...' : ''}`));
                return;
            }
            ...
            if (invalidDelete.length > 0) {
                res.status(400).json((0, codeMessage_1.errorResponse)("INVALID_IMAGES", `Some images to delete don't exist in product: ${invalidDelete.slice(0, 3).join(', ')}${invalidDelete.length > 3 ? '...' : ''}`));
                return;
            }
            // Validate keep + uploaded images <= 6
            if (keep.length + uploadedImageUrls.length > 6) {
                res.status(400).json((0, codeMessage_1.errorResponse)("TOO_MANY_IMAGES", `Maximum 6 images allowed. You have ${keep.length} kept + ${uploadedImageUrls.length} new = ${keep.length + uploadedImageUrls.length}`));
                return;
            }
        ...
        if (finalImages.length > 6) {
            res.status(400).json((0, codeMessage_1.errorResponse)("TOO_MANY_IMAGES", "Maximum 6 images allowed"));
            return;
        }
        // ============================================
        ...
        if (isUsingOldFormat) {
            ...
            if (shouldDelete && currentVideo.length > 0) {
                ...
                if (!currentVideo.includes(keep)) {
                    res.status(400).json((0, codeMessage_1.errorResponse)("INVALID_VIDEO", "Video to keep doesn't exist in product"));
                    return;
                }
                finalVideo = [keep];
        ...
        if (finalVideo.length > 1) {
            res.status(400).json((0, codeMessage_1.errorResponse)("TOO_MANY_VIDEOS", "Maximum 1 video allowed"));
            return;
        }
        // Validate uploaded videos
        if (uploadedVideoUrls.length > 1) {
            res.status(400).json((0, codeMessage_1.errorResponse)("TOO_MANY_VIDEOS", "Maximum 1 video can be uploaded at a time"));
            return;
        }
        // ============================================
        // ✅ BUILD UPDATE DATA
        ...
        redis_baseQueue_1.productIndexQueue.add("indexProduct", { productId: product.id }).catch(console.error);
        ...
        const scheduleSafe = updated.productSchedule
            ...
            : null;
        const computedIsLive = (0, exports.computeIsLive)(scheduleSafe, updated.isLive);
        const cacheKey = redisCacheTiming_1.CACHE_KEYS.PRODUCT_DETAIL(product.id);
        ...
        try {
            ...
            await redis_1.redisProducts.setEx(cacheKey, 3600, cacheValue);
        }
        catch (error) {
            try {
                // Method 2: set with EX option
                await redis_1.redisProducts.set(cacheKey, cacheValue, { EX: 3600 });
            }
            catch (error2) {
                try {
                    // Method 3: Just set without expiry
                    await redis_1.redisProducts.set(cacheKey, cacheValue);
        ...
        setTimeout(async () => {
            try {
                const totalCount = await prisma_1.default.product.count({ where: { vendorId: req.user.id } });
                const limit = 20;
                const totalPages = Math.ceil(totalCount / limit);
                // Warm up first 3 pages
                const pagesToWarm = Math.min(3, totalPages);
                await Promise.all(Array.from({ length: pagesToWarm }, (_, i) => service.getAllVendorProducts(i * limit, limit)));
            }
            catch (error) {
        ...
        // ✅ SUCCESS RESPONSE
        // ============================================
        res.status(200).json((0, codeMessage_1.successResponse)("PRODUCT_UPDATED", "Product updated successfully", {
            ...updated,
            images: finalImages,
            video: finalVideo,
        }));
        ...
        await (0, clearCaches_1.clearProductCache)(productId);
        res.status(500).json((0, codeMessage_1.errorResponse)("SERVER_ERROR", "Something went wrong"));
    }
};
...
const archiveProduct = async (req, res) => {
    ...
    try {
        ...
        if (!parsed.success) {
            res.status(400).json((0, codeMessage_1.errorResponse)("VALIDATION_ERROR", "archived must be a boolean"));
            return;
        }
        const { archived } = parsed.data;
        const product = await prisma_1.default.product.findUnique({ where: { id: productId }, include: { productSchedule: true } });
        if (!product || product.vendorId !== req.user.id) {
            res.status(403).json((0, codeMessage_1.errorResponse)("FORBIDDEN", "Unauthorized or product not found"));
            return;
        }
        ...
        redis_baseQueue_1.productIndexQueue.add("indexProduct", { productId }).catch(console.error);
        ...
        const computedIsLive = (0, exports.computeIsLive)(scheduleSafe, updated.isLive);
        const cacheKey = redisCacheTiming_1.CACHE_KEYS.PRODUCT_DETAIL(productId);
        await redis_1.redisProducts.set(cacheKey, JSON.stringify({ ...updated, isLive: computedIsLive }));
        const service = new vendorDashboard_service_1.VendorDashboardService(req.user.id);
        await service.invalidateCache();
        const totalCount = await prisma_1.default.product.count({ where: { vendorId: req.user.id } });
        const limit = 20;
        ...
        res.status(200).json((0, codeMessage_1.successResponse)("PRODUCT_ARCHIVED", `Product successfully ${archived ? "archived" : "unarchived"}`, updated));
    }
    catch (err) {
...
const deleteProduct = async (req, res) => {
    ...
    try {
        ...
        await (0, product_service_1.clearProductFromCarts)(product.id);
        res.json((0, codeMessage_1.successResponse)("PRODUCT_DELETED", "Product deleted successfully"));
    }
    catch (err) {
...
const getSearchSuggestions = async (req, res) => {
    try {
        ...
        // 1️⃣ Validate
        if (!q || q.length < 2) {
            return res.status(400).json((0, codeMessage_1.errorResponse)("INVALID_QUERY", "Please enter at least 2 characters"));
        }
        const query = q.toLowerCase();
        const keywords = query.split(/\s+/).filter(Boolean);
        const isSingleWord = keywords.length === 1;
        const cacheKey = redisCacheTiming_1.CACHE_KEYS.SUGGESTIONS(query);
        const ttl = redisCacheTiming_1.CACHE_TTLS.SUGGESTIONS; // e.g. 30–60s
        // 2️⃣ Cache
        const cached = await redis_1.redisSearch.get(cacheKey);
        if (cached) {
            return res.json((0, codeMessage_1.successResponse)("SUGGESTIONS_FETCHED", "Suggestions retrieved successfully", {
                query,
        ...
        if (isSingleWord) {
            results = await prisma_1.default.product.findMany({
                where: {
                    archived: false,
                    name: {
                        contains: query,
        ...
        // 6️⃣ Cache
        await redis_1.redisSearch.set(cacheKey, JSON.stringify(suggestions), { EX: ttl });
        return res.json((0, codeMessage_1.successResponse)("SUGGESTIONS_FETCHED", "Suggestions retrieved successfully", {
            query,
            results: suggestions,
            fromCache: false,
        ...
        return res
            .status(500)
            .json((0, codeMessage_1.errorResponse)("SERVER_ERROR", "Failed to get suggestions"));
    }
};
...
const searchProducts = async (req, res) => {
    try {
        ...
        if (!q) {
            return res
                .status(400)
                .json((0, codeMessage_1.errorResponse)("INVALID_QUERY", "Query parameter 'q' is required"));
        }
        ...
        if (cached) {
            ...
            return res.json((0, codeMessage_1.successResponse)("SEARCH_SUCCESS", "Products fetched successfully", {
                corrected,
    ...
    `, corrected, `%${corrected}%`);
        ...
        // ---------------- CACHE ----------------
        await redis_1.redisSearch.set(cacheKey, JSON.stringify(responseData), { EX: redisCacheTiming_1.CACHE_TTLS.SEARCH });
        return res.json((0, codeMessage_1.successResponse)("SEARCH_SUCCESS", "Products fetched successfully", responseData));
    }
    catch (err) {
        console.error("❌ Search error:", err);
        return res.status(500).json((0, codeMessage_1.errorResponse)("SERVER_ERROR", "Internal server error"));
    }
};
...
const getMostPopularProducts = async (req, res) => {
    try {
        ...
        const ttl = redisCacheTiming_1.CACHE_TTLS.PRODUCTS_MOST_POPULAR;
        const cached = await redis_1.redisProducts.get(cacheKey);
        let products = [];
        let totalCount = 0;
        if (cached) {
        ...
        `);
            ...
            res.setHeader("X-Cache", "MISS");
            // Cache results
            await redis_1.redisProducts.set(cacheKey, JSON.stringify({
                data: products,
        ...
        return res.json((0, codeMessage_1.successResponse)("POPULAR_PRODUCTS_FETCHED", "Most popular products fetched successfully", products, pagination));
    }
    catch (err) {
        console.error("Get most popular products error:", err);
        return res
            .status(500)
            .json((0, codeMessage_1.errorResponse)("SERVER_ERROR", "Failed to fetch most popular products"));
    }
};
