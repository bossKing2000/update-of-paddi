...
class DeliveryAssignmentService {
    static async expireOldBroadcasts() {
        const now = new Date();
        const expired = await prismaClient_1.default.deliveryBroadcast.findMany({
            where: { status: "PENDING", expiresAt: { lt: now } },
        });
        for (const b of expired) {
            await prismaClient_1.default.deliveryBroadcast.update({
                where: { id: b.id },
                data: { status: "EXPIRED" },
            });
    ...
    static async findAvailableDrivers(lat, lng) {
        const activeDrivers = await prismaClient_1.default.deliveryPerson.findMany({
            where: {
                isOnline: true,
                status: "ACTIVE",
        ...
        const driversWithDistance = activeDrivers.map((driver) => {
            ...
            const distance = (0, haversine_distance_1.default)({ lat, lon: lng }, { lat: driverLat, lon: driverLng }) / 1000; // convert to km
            return {
    ...
    static async assignOrder(orderId, driverId) {
        ...
        const vendorAddress = await prismaClient_1.default.address.findFirst({
        ...
        const drivers = await prismaClient_1.default.deliveryPerson.findMany({
            where: {
                isOnline: true,
                status: "ACTIVE",
                user: { isEmailVerified: true },
        ...
        const driversWithInfo = await Promise.all(drivers.map(async (driver) => {
            const distanceKm = (0, haversine_distance_1.default)({ lat: vendorLocation.lat, lon: vendorLocation.lon }, { lat: driver.latitude || 0, lon: driver.longitude || 0 }) / 1000;
            const activeAssignments = await prismaClient_1.default.deliveryAssignment.count({
                where: {
        ...
        if (driverId) {
            ...
            const batchId = manualDriver.activeAssignments > 0
                ? `batch_${Date.now()}_${manualDriver.id}`
                : null;
            const assignment = await prismaClient_1.default.deliveryAssignment.create({
                data: {
                    orderId: order.id,
            ...
            const distanceKm = (0, haversine_distance_1.default)({ lat: vendorLocation.lat, lon: vendorLocation.lon }, { lat: manualDriver.latitude || 0, lon: manualDriver.longitude || 0 }) / 1000;
        ...
        const broadcast = await prismaClient_1.default.deliveryBroadcast.create({
            data: {
                orderId: order.id,
                driverIds,
                expiresAt,
    ...
    static async acceptAssignment(assignmentId, driverId) {
        const assignment = await prismaClient_1.default.deliveryAssignment.update({
     ...
     */
    static async handleDecline(assignmentId) {
        const assignment = await prismaClient_1.default.deliveryAssignment.update({
            where: { id: assignmentId },
    ...
    static async getActiveAssignmentsForDriver(driverId) {
        ...
        const deliveryPerson = await prismaClient_1.default.deliveryPerson.findUnique({
            where: { userId: driverId },
        });
        if (!deliveryPerson)
            throw new Error("Driver not found");
        // ✅ Now query by the actual deliveryPerson.id
        return prismaClient_1.default.deliveryAssignment.findMany({
            where: {
     ...
     */
    static async broadcastDriverLocation(driverId) {
        const assignments = await prismaClient_1.default.deliveryAssignment.findMany({
            where: {
    ...
    static async updateStatus(assignmentId, driverId, status) {
        ...
        const assignment = await prismaClient_1.default.deliveryAssignment.findUnique({
            where: { id: assignmentId },
            include: { order: true },
        });
        if (!assignment)
        ...
        const updated = await prismaClient_1.default.deliveryAssignment.update({
            where: { id: assignmentId },
            data: { status },
            include: { order: true },
        });
     ...
     */
    static async getDriverHistory(driverId, limit = 50) {
        return prismaClient_1.default.deliveryAssignment.findMany({
            where: {
                deliveryPersonId: driverId,
    ...
    static async logDriverLocation(driverId, latitude, longitude) {
        await prismaClient_1.default.driverLocationLog.create({
            data: {
                driverId,
                latitude,
                longitude,
    ...
    static async getDriverAnalytics(driverId) {
        ...
        if (!deliveryPerson)
            throw new Error("Driver not found");
        const completed = await prismaClient_1.default.deliveryAssignment.count({
            where: { deliveryPersonId: deliveryPerson.id, status: "DELIVERED" },
        });
        const failed = await prismaClient_1.default.deliveryAssignment.count({
            where: {
                deliveryPersonId: deliveryPerson.id,
